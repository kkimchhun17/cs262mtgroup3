            ...
            <div class="col-12 col-md-5 col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 p-4 bg-light h-50 d-flex flex-column justify-content-between align-items-center" style="min-height: 580px;">
                    ...
                    <div class="mt-2 pt-3 border-top w-100 text-start text-muted">
                        ...
                        <span id="preview-status" class="text-xs text-success fw-semibold mt-2 d-block">
                            {{ $dish->image_url ? 'Showing saved database image asset.' : '' }}
                        </span>
                    </div>
                </div>
            </div>
            <div class="col-12 col-md-7 col-lg-8 d-flex flex-column justify-content-between">
                <div class="w-100">
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control form-control-lg bg-light border-0 py-3 rounded-3 shadow-sm @error('name') is-invalid @enderror" id="input-name" name="name" placeholder="food name" value="{{ old('name', $dish->name) }}" required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control bg-light border-0 p-3 rounded-3 shadow-sm @error('description') is-invalid @enderror" id="input-description" name="description" rows="7" placeholder="description for the food" required>{{ old('description', $dish->description) }}</textarea>
                        @error('description')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="row g-4 mb-4">
                        ...
                        <div class="col-12 col-sm-7">
                            ...
                            <input type="number" class="form-control form-control-lg bg-light border-0 py-3 rounded-3 shadow-sm @error('duration') is-invalid @enderror" id="input-duration" name="duration" placeholder="e.g., 20" min="1" value="{{ old('duration', $dish->duration) }}" required>
                            @error('duration')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
